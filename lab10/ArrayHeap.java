...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        /** TODO: Your code here. */
        while(index != 0 && contents[parentIndex(index)] != null
                && getNode(parentIndex(index)).myPriority > getNode(index).myPriority){
    ...
    private void sink(int index) {
        ...
        while(2 * index <= size){
            int j = 2 * index;
            if(contents[index].myPriority < contents[min(j, j + 1)].myPriority){
                break;
            }
            int k = min(j, j + 1);
            swap(index, k);
